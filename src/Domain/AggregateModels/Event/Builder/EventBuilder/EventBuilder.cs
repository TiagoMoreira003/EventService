...
namespace EventService.Domain.AggregateModels.Event.Builder.EventBuilder
{
    ...
    internal class EventBuilder : IEventBuilder
    {
        ...
        /// <returns></returns>
        public IEventBuilder NewEvent(EventDate eventDate, MusicType musicType, Location location, string description, Guid tenantId, string name, Guid imageId, List<string> artists)
        {
            Event = new Event(eventDate, musicType, location, description, tenantId, name, imageId, artists);
            return this;
        }
